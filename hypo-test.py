"""
Hypothesis Testing Framework for Navier-Stokes Singularities
===========================================================

This script uses the 3D solver to systematically test hypotheses
about the nature of Navier-Stokes singularities. It performs a
parameter sweep over initial conditions and analyzes the collected
results to uncover deeper patterns.
"""
...
class FilterDiscovery:
    ...
    def discover_filter(self,u0,v0,w0,objective_type='peak_enstrophy'):
        ...
        result=differential_evolution(objective,bounds,maxiter=5,popsize=5,disp=True) # Faster for sweep
...
def run_single_experiment(initial_condition_params):
    ...
    # Save plots
    plot_filename = f"hypo_test_{ic_type}_sep_{initial_condition_params.get('separation', 'N/A'):.2f}"
...
def plot_sweep_results(sweep_params, sweep_results):
    fig, axes = plt.subplots(1, 2, figsize=(15, 6))
    fig.suptitle("Hypothesis Test Sweep Results", fontsize=16)
    ...
    plt.savefig("hypo_test_sweep_summary.png")
    plt.show()
...
def plot_results(times, max_coeffs, results, blow_up_law, filename):
    ...
    plt.tight_layout(rect=[0,0.03,1,0.95]);plt.savefig(filename,dpi=120);plt.close(fig)
...
if __name__ == "__main__":
    ...
    separations_to_test = [np.pi/1.5, np.pi/2, np.pi/3, np.pi/4]
    sweep_results = []
    ...
    if len(sweep_results) > 1:
        plot_sweep_results(separations_to_test, sweep_results)

    print("\n\nHypothesis testing complete.")
    print("Check the generated image files for results of each run and the sweep summary.")
